...
public class NameGenerator {
  ...
  public void refresh() {
    try {
      ...
      StringReader sReader = new StringReader(syllabes);

      bufRead = new BufferedReader(sReader);
      line = "";
  ...
  public String compose(int syls) {
    if (syls > 2 && mid.size() == 0) {
      throw new RuntimeException("You are trying to create a name with more than 3 parts, which requires middle parts, " +
              "which you have none in the file " + syllabes + ". You should add some. Every word, " +
              "which doesn't have + or - for a prefix is counted as a middle part.");
    }
    if (pre.size() == 0) {
      throw new RuntimeException("You have no prefixes to start creating a name. add some and use \"-\" " +
              "prefix, to identify it as a prefix for a name. (example: -asd)");
    }
    if (sur.size() == 0) {
      throw new RuntimeException("You have no suffixes to end a name. add some and use \"+\" prefix, " +
              "to identify it as a suffix for a name. (example: +asd)");
    }
    if (syls < 1) {
    ...
    if (vocalLast(pureSyl(pre.get(a))) && allowVocs(mid) == false) {
      throw new RuntimeException("Expecting \"middle\" part that allows last character of prefix to be a vocal, " +
              "but there is none. You should add one, or remove requirements that cannot be fulfilled.. " +
              "the prefix used, was : \"" + pre.get(a) + "\", which" +
              "means there should be a part available, that has \"-v\" requirement or no requirements " +
              "for previous syllables at all.");
    }
    ...
    if (consonantLast(pureSyl(pre.get(a))) && allowCons(mid) == false) {
      throw new RuntimeException("Expecting \"middle\" part that allows last character of prefix to be a consonant, " +
              "but there is none. You should add one, or remove requirements that cannot be fulfilled.. " +
              "the prefix used, was : \"" + pre.get(a) + "\", which" +
              "means there should be a part available, that has \"-c\" requirement or no requirements " +
              "for previous syllables at all.");
    }
    ...
    for (int i = 0; i < b.length - 2; i++) {
      ...
      while (expecting == 1 && vocalFirst(pureSyl(mid.get(b[i]))) == false || expecting == 2 &&
      ...
      if (vocalLast(pureSyl(mid.get(b[i]))) && allowVocs(mid) == false && syls > 3) {
        throw new RuntimeException("Expecting \"middle\" part that allows last character of last syllable to be a vocal, " +
                "but there is none. You should add one, or remove requirements that cannot be fulfilled.. " +
                "the part used, was : \"" + mid.get(b[i]) + "\", which " +
                "means there should be a part available, that has \"-v\" requirement or no requirements " +
                "for previous syllables at all.");
      }
      ...
      if (i == b.length - 3) {
        if (vocalLast(pureSyl(mid.get(b[i]))) && allowVocs(sur) == false) {
          throw new RuntimeException("Expecting \"suffix\" part that allows last character of last syllable to be a vocal, " +
                  "but there is none. You should add one, or remove requirements that cannot be fulfilled.. " +
                  "the part used, was : \"" + mid.get(b[i]) + "\", which " +
                  "means there should be a suffix available, that has \"-v\" requirement or no requirements " +
                  "for previous syllables at all.");
        }
        ...
        if (consonantLast(pureSyl(mid.get(b[i]))) && allowCons(sur) == false) {
          throw new RuntimeException("Expecting \"suffix\" part that allows last character of last syllable to be a consonant, " +
                  "but there is none. You should add one, or remove requirements that cannot be fulfilled.. " +
                  "the part used, was : \"" + mid.get(b[i]) + "\", which " +
                  "means there should be a suffix available, that has \"-c\" requirement or no requirements " +
                  "for previous syllables at all.");
        }
      }
